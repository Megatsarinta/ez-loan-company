...
0 -20 Td
(Interest Rate: ${(Number(loan.interest_rate) * 100).toFixed(1)}% per month) Tj
0 -20 Td
...
0 -20 Td
(Interest Rate: ${(Number(contract?.interest_rate ?? loan.interest_rate) * 100).toFixed(1)}% per month) Tj
0 -20 Td
...
// Repayment = (Principal / Term) + (Principal × Monthly Rate)
function calculateMonthlyPayment(
  ...
  const interestPerMonth = principal * monthlyRateDecimal
  return (principalPerMonth + interestPerMonth).toFixed(2)
}
