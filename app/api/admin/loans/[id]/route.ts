...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.log('[v0] Fetching loan with ID:', loanId);

    // Try to fetch from loan_applications first with JOIN to users table
    ...
    if (loanAppData && !appError) {
      ...
      // 🔴 CRITICAL: Log detailed data for debugging
      console.log('[v0] Complete loan record:', {
        id: loanAppData.id,
        document_number: loanAppData.document_number,
        user_id: loanAppData.user_id,
        borrower_name: loanAppData.users?.full_name,
        amount_requested: loanAppData.amount_requested,
      ...
      console.log('[v0] Returning transformed loan data - Document:', transformedData.document_number, 'ID:', transformedData.id, 'Borrower:', transformedData.borrower_name);
    ...
    // Fallback: try loans table with JOIN to users
    ...
    if (loansData && !loansError) {
      console.log('[v0] Found in loans table');
      ...
      const transformedData = {
        ...
        borrower_id_number: loansData.users?.id_card_number || loansData.id_number || 'N/A',
    ...
    console.error('[v0] Loan not found with ID:', loanId);
    return NextResponse.json(
