...
export async function GET(req: NextRequest) {
  try {
    ...
    // Transform data to match expected structure (score: cibil_score preferred)
    const transformedUsers = (users || []).map((user: any) => ({
      ...
      status: user.is_banned === true ? 'disabled' : 'active',
      // 🔴 FIX: Use the new location columns we added
      registrationArea: user.registration_location || `Unknown, IP: ${user.ip_address || 'N/A'}`,
