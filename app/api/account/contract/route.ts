...
import { getLatestLoanApplicationByUserId, getBankDetails, getPersonalInfo } from '@/lib/db-operations';
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Get user's most recent loan (direct PG to avoid schema cache)
    ...
    const loanApp = loanResult.data as any;
    let personalInfo = loanApp.personal_info && typeof loanApp.personal_info === 'object' ? loanApp.personal_info : {};
    if (!personalInfo.id_card_number && !personalInfo.id_number) {
      const piResult = await getPersonalInfo(parseInt(userId));
      if (piResult.success && piResult.data) {
        const row = piResult.data as any;
        personalInfo = { ...personalInfo, id_number: row.id_number, id_card_number: row.id_number, full_name: row.full_name || personalInfo.full_name, phone_number: row.phone_number || personalInfo.phone_number };
      }
    }
    ...
    const bankName = (bankResult.success && bankResult.data && (bankResult.data as any).bank_name)
      ? (bankResult.data as any).bank_name
    ...
    return NextResponse.json({
      ...
      contract: {
        ...
        loan_amount: formatINR(loanApp.amount_requested),
        interest_rate: `${(Number(loanApp.interest_rate) * 100).toFixed(1)}% per month`,
