...
async function generateExcel(loanData: any, schedule: any[], documentNumber: string) {
  ...
  const mainData = [
    ...
    ['', '═ LOAN SUMMARY ═════════════════════════════════════════════════════', '', '', '', '', '', ''],
    ['Loan Amount:', `₹${Number(loanData.loanAmount).toLocaleString('en-IN')}`, '', '', '', '', '', ''],
    ['Interest Rate:', `${(Number(loanData.interestRate) * 100).toFixed(2)}% per month`, '', '', '', '', '', ''],
    ['Loan Term:', `${loanData.loanTerm} months`, '', '', '', '', '', ''],
    ['Start Date:', loanData.startDate, '', '', '', '', '', ''],
    ['Monthly EMI:', `₹${schedule[0]?.total?.toFixed(2) || '0.00'}`, '', '', '', '', '', ''],
    ...
    ['•', 'Contact support@easyloancompany.com for assistance', '', '', '', '', '', ''],
    ['', '', '', '', '', '', '', ''],
    
    // COMPANY FOOTER
    ['EASY LOAN COMPANY PVT LTD', '', '', '', '', '', '', ''],
    ['Maker Maxity, Bandra Kurla Complex, Mumbai - 400051, Maharashtra, India', '', '', '', '', '', '', ''],
    ['Phone: +91 22 6789 1234 | Email: support@easyloancompany.com', '', '', '', '', '', '', ''],
    ['Registered with RBI, MCA, and CIBIL', '', '', '', '', '', '', '']
  ]
    ...
    for (let C = 0; C <= 7; C++) {
      ...
      if (mainSheet[cell]) {
        ...
        if (isAmountCol && (C === 2 || C === 3 || C === 4 || C === 5 || C === 6 || C === 7)) {
          mainSheet[cell].z = Number(C) === 0 ? '0' : '₹#,##0.00'
        }
      }
    }
  }
...
function generateHTML(loanData: any, schedule: any[], format: string) {
  const html = `
    ...
    <body>
      <div class="header">
        <h1>EasyLoan - Repayment Schedule</h1>
      ...
      <div class="summary">
        <div class="summary-item">
          ...
          <div class="summary-value">₹${Number(loanData.loanAmount).toLocaleString('en-IN')}</div>
        </div>
        <div class="summary-item">
      ...
      <div class="footer">
        <p><strong>EASY LOAN COMPANY PVT LTD</strong></p>
        <p>Maker Maxity, Bandra Kurla Complex, Mumbai - 400051, Maharashtra, India</p>
