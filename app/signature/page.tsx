...
export default function SignaturePage() {
      ...
      try {
        ...
        if (appResponse.ok) {
          ...
          if (appData.application) {
            ...
            // Require bank information before signature
  ...
  useEffect(() => {
    ...
    ctx.strokeStyle = '#E5E7EB' // matches --color-border
    ctx.lineWidth = 1
  ...
  const draw = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    ctx.strokeStyle = '#111827' // matches --color-text-primary
    ctx.lineWidth = 2
    ctx.lineCap = 'round'
    ctx.lineJoin = 'round'
    ctx.lineTo(x, y)
    ...
    <div className="min-h-screen bg-gradient-to-b from-[var(--color-bg-main)] to-[var(--color-border)] pb-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--color-text-secondary)]">Sign Agreement</p>
              </div>
            </div>
          </div>
      ...
      <main className="px-4 py-6 max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-text-secondary)]">Review and sign to complete your application</p>
        </div>
        ...
        <div className="bg-[var(--color-bg-surface)] border-2 border-[var(--color-border)] rounded-2xl p-6 shadow-lg mb-6 font-serif">
          {/* Contract Header */}
          <div className="text-center mb-6 pb-4 border-b-2 border-[var(--color-accent-500)]/20">
            <h2 className="text-xl md:text-2xl font-bold text-[var(--color-accent-500)]">LOAN AGREEMENT</h2>
            <p className="text-xs text-[var(--color-text-secondary)] mt-1">Document No: {application?.document_number} • RBI Regd NBFC</p>
          </div>

          {/* Loan Summary */}
          <div className="bg-gradient-to-r from-orange-50 to-green-50 border border-[var(--color-accent-500)]/20 rounded-xl p-5 mb-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-[var(--color-text-secondary)]">Loan Amount</p>
                <p className="text-lg font-bold text-[var(--color-accent-500)]">{formatCurrency(contractData.amount)}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--color-text-secondary)]">Loan Term</p>
                <p className="text-lg font-bold text-[var(--color-text-primary)]">{contractData.term} months</p>
              </div>
              <div>
                <p className="text-xs text-[var(--color-text-secondary)]">Monthly EMI</p>
                <p className="text-lg font-bold text-[var(--color-secondary-600)]">{formatCurrency(contractData.monthlyPayment)}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--color-text-secondary)]">Interest Rate</p>
                <p className="text-lg font-bold text-[var(--color-text-primary)]">{contractData.interestRate.toFixed(1)}% per month</p>
              </div>
            </div>
          </div>
          ...
          <Button
            ...
            className="w-full bg-gradient-to-r from-[var(--color-accent-500)] to-[var(--color-secondary-600)] hover:from-[var(--color-accent-600)] hover:to-[var(--color-secondary-500)] text-[var(--color-bg-surface)] font-semibold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 mb-6"
          >
          ...
          <div className="space-y-4 mb-6">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gradient-to-r from-[var(--color-accent-500)] to-[var(--color-secondary-600)] rounded-full flex items-center justify-center">
                <Pen className="w-4 h-4 text-[var(--color-bg-surface)]" />
              </div>
              <h3 className="text-lg font-semibold text-[var(--color-text-primary)]">Borrower's Signature</h3>
            </div>
            
            <div className="border-2 border-[var(--color-border)] rounded-xl overflow-hidden bg-[var(--color-bg-surface)]">
              <canvas
                ref={canvasRef}
                width={400}
                height={200}
            ...
            <p className="text-sm text-[var(--color-text-secondary)] text-center">Draw your signature above using your finger or mouse</p>
          </div>

          {/* Reset Button */}
          <Button
            onClick={resetSignature}
            className="w-full border-2 border-[var(--color-border)] hover:border-[var(--color-accent-500)] text-[var(--color-text-primary)] hover:text-[var(--color-accent-500)] bg-[var(--color-bg-surface)] hover:bg-[var(--color-bg-main)] font-medium py-3 rounded-xl transition-all duration-300 mb-4"
          >
          ...
          {/* Legal Notice */}
          <div className="bg-[var(--color-bg-main)] border border-[var(--color-border)] rounded-xl p-4 mt-6">
      ...
      {showContract && contractFullText && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-2 sm:p-4 z-50 backdrop-blur-sm">
          <div className="bg-[var(--color-bg-surface)] rounded-2xl shadow-2xl max-w-4xl w-full max-h-[98vh] sm:max-h-[95vh] md:max-h-[90vh] overflow-hidden">
            
            {/* Modal Header with X button */}
            <div className="bg-gradient-to-r from-[var(--color-accent-500)] to-[var(--color-secondary-600)] text-[var(--color-bg-surface)] px-3 py-3 sm:px-6 sm:py-4 md:px-8 md:py-6 relative">
              {/* X Close Button - Always visible */}
              <button
                onClick={() => setShowContract(false)}
                className="absolute top-3 right-3 sm:top-4 sm:right-4 md:top-6 md:right-6 text-[var(--color-bg-surface)]/80 hover:text-[var(--color-bg-surface)] bg-black/10 hover:bg-black/20 rounded-full p-1 sm:p-1.5 transition-colors z-20"
              >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 pr-8">
                ...
                <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                  ...
                  <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-[var(--color-bg-surface)]/10 p-1 sm:p-1.5 md:p-2 rounded-lg backdrop-blur-sm flex-shrink-0">
                    <img 
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[var(--color-bg-surface)]/90 text-xs sm:text-sm mt-0.5 truncate">
                ...
                <div className="flex items-center justify-start sm:justify-end">
                  <div className="bg-[var(--color-bg-surface)]/10 rounded-lg px-2 py-1 sm:px-3 sm:py-1.5 md:px-4 md:py-2">
                    <p className="text-[10px] sm:text-xs font-medium text-[var(--color-bg-surface)]/80">Document No.</p>
                    <p className="text-xs sm:text-sm md:text-base lg:text-lg font-mono font-bold text-[var(--color-bg-surface)] break-all">
            ...
            <div className="p-3 sm:p-4 md:p-6 lg:p-8 overflow-y-auto max-h-[calc(98vh-140px)] sm:max-h-[calc(95vh-160px)] md:max-h-[calc(90vh-180px)] bg-[var(--color-bg-main)]">
              ...
              <div className="relative">
                ...
                <div className="relative space-y-4 sm:space-y-5 md:space-y-6 lg:space-y-8 font-serif text-gray-800 text-xs sm:text-sm md:text-base">
                  ...
                  {contractHeader && (
                    <div className="bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 lg:p-6 shadow-sm">
                      <h3 className="text-sm sm:text-base md:text-lg font-bold text-[var(--color-accent-500)] mb-2 sm:mb-3 md:mb-4 pb-1 sm:pb-2 border-b border-[var(--color-border)]">
                        PARTIES TO THIS AGREEMENT
                      </h3>
                      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 sm:gap-6 md:gap-8">
                        {/* Lender Section */}
                        <div className="space-y-1 flex-1">
                          <p className="text-[10px] sm:text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">LENDER</p>
                          <p className="font-bold text-gray-900 text-xs sm:text-sm md:text-base">{COMPANY_INFO.name}</p>
                          <p className="text-[10px] sm:text-xs md:text-sm text-[var(--color-text-secondary)] break-words">{COMPANY_INFO.registeredOffice.address}</p>
                          <p className="text-[10px] sm:text-xs md:text-sm text-[var(--color-text-secondary)] break-words">{COMPANY_INFO.registeredOffice.area}</p>
                          <p className="text-[10px] sm:text-xs md:text-sm text-[var(--color-text-secondary)] break-words">{COMPANY_INFO.registeredOffice.city}, {COMPANY_INFO.registeredOffice.state} - {COMPANY_INFO.registeredOffice.pincode}</p>
                        ...
                        <div className="space-y-1 flex-1 text-left sm:text-right">
                          <p className="text-[10px] sm:text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">BORROWER</p>
                          <p className="font-bold text-gray-900 text-xs sm:text-sm md:text-base break-words">
                            {contractHeader.borrower_name}
                          </p>
                          <p className="text-[10px] sm:text-xs md:text-sm text-[var(--color-text-secondary)] break-words">
                  ...
                  {/* Loan Terms Summary */}
                  <div className="bg-gradient-to-br from-orange-50 to-green-50 border border-[var(--color-accent-500)]/20 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 lg:p-6">
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-[var(--color-accent-500)] mb-2 sm:mb-3 md:mb-4">
                      LOAN TERMS SUMMARY
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)] sm:border-0">
                        <span className="text-xs font-medium text-[var(--color-text-secondary)]">Principal Amount:</span>
                        <span className="text-sm font-bold text-[var(--color-accent-500)]">{contractHeader?.loan_amount}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)] sm:border-0">
                        <span className="text-xs font-medium text-[var(--color-text-secondary)]">Interest Rate:</span>
                        <span className="text-sm font-bold">{contractHeader?.interest_rate} per month</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)] sm:border-0">
                        <span className="text-xs font-medium text-[var(--color-text-secondary)]">Loan Term:</span>
                        <span className="text-sm font-bold">{contractHeader?.loan_period}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 border-b border-[var(--color-border)] sm:border-0">
                        <span className="text-xs font-medium text-[var(--color-text-secondary)]">Monthly EMI:</span>
                        <span className="text-sm font-bold text-[var(--color-secondary-600)]">{formatCurrency(contractData?.monthlyPayment || 0)}</span>
                      </div>
                      <div className="flex justify-between items-center py-2 sm:col-span-2">
                        <span className="text-xs font-medium text-[var(--color-text-secondary)]">Total Repayment:</span>
                        <span className="text-sm font-bold text-[var(--color-secondary-600)]">
                  ...
                  <div className="bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 lg:p-6 shadow-sm">
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-[var(--color-accent-500)] mb-3 sm:mb-4 md:mb-5 lg:mb-6 pb-1 sm:pb-2 border-b border-[var(--color-border)]">
                  ...
                  {/* Signature Section */}
                  <div className="bg-[var(--color-bg-surface)] border border-[var(--color-border)] rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 lg:p-6 shadow-sm mt-4 sm:mt-5 md:mt-6 lg:mt-8">
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-[var(--color-accent-500)] mb-3 sm:mb-4 md:mb-5 lg:mb-6 pb-1 sm:pb-2 border-b border-[var(--color-border)]">
                      SIGNATURE PAGE
                    </h3>
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-6 sm:gap-8 md:gap-12">
                      ...
                      <div className="flex-1">
                        <p className="text-[11px] sm:text-xs md:text-sm font-semibold text-[var(--color-text-secondary)] mb-2 sm:mb-3 md:mb-4">BORROWER'S SIGNATURE</p>
                        ...
                        <div className="mb-2 sm:mb-3 md:mb-4" style={{ minHeight: '60px' }}>
                          {application?.signature_url ? (
                            ...
                            <p className="text-[var(--color-text-secondary)] text-[10px] sm:text-xs italic">(To be signed above)</p>
                          )}
                        </div>
                        
                        <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)] break-words">Printed Name: {contractHeader?.borrower_name}</p>
                        <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)]">
                      ...
                      <div className="flex-1 relative">
                        ...
                        <div className="relative z-10">
                          <p className="text-[11px] sm:text-xs md:text-sm font-semibold text-[var(--color-text-secondary)] mb-2 sm:mb-3 md:mb-4">LENDER'S SIGNATURE</p>
                          
                          <div className="space-y-1">
                            <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)]">Printed Name: {COMPANY_INFO.keyPersonnel.ceo.name}</p>
                            <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)]">Title: {COMPANY_INFO.keyPersonnel.ceo.designation}</p>
                            <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)]">
                              Date: {new Date().toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
                            </p>
                          </div>
                    ...
                    <div className="mt-4 sm:mt-5 md:mt-6 text-center text-[10px] sm:text-xs text-[var(--color-text-secondary)] border-t border-[var(--color-border)] pt-3 sm:pt-4">
                  ...
                  <div className="bg-gradient-to-r from-orange-50 to-green-50 border border-[var(--color-accent-500)]/20 rounded-lg sm:rounded-xl p-3 sm:p-4 text-center">
                    <p className="text-[11px] sm:text-xs md:text-sm text-[var(--color-text-primary)] italic font-medium break-words">
            ...
            <div className="bg-[var(--color-bg-main)] px-3 py-3 sm:px-4 sm:py-3 md:px-6 md:py-4 lg:px-8 lg:py-4 border-t border-[var(--color-border)] hidden sm:flex flex-col sm:flex-row justify-end gap-2 sm:gap-3">
              <button
                onClick={() => setShowContract(false)}
                className="bg-[var(--color-bg-surface)] border-2 border-[var(--color-accent-500)] text-[var(--color-accent-500)] hover:bg-[var(--color-accent-100)] font-semibold px-4 py-2 sm:px-5 sm:py-2 md:px-6 md:py-2 text-xs sm:text-sm md:text-base rounded-lg transition-colors w-full sm:w-auto order-2 sm:order-1"
              >
                Back to Contract
              </button>
              <button
                ...
                className="bg-gradient-to-r from-[var(--color-accent-500)] to-[var(--color-secondary-600)] hover:from-[var(--color-accent-600)] hover:to-[var(--color-secondary-500)] text-[var(--color-bg-surface)] font-semibold px-6 py-2 sm:px-8 sm:py-2 md:px-10 md:py-2 text-xs sm:text-sm md:text-base rounded-lg shadow-lg hover:shadow-xl transition-all w-full sm:w-auto order-1 sm:order-2"
              >
                Sign Contract
              </button>
            </div>
      ...
      {/* Loading Overlay */}
      {isSubmitting && (
        <div className="fixed inset-0 bg-[var(--color-bg-surface)]/80 flex items-center justify-center z-50">
          <div className="text-center">
            <div className="inline-block w-12 h-12 border-4 border-[var(--color-accent-500)] border-t-transparent rounded-full animate-spin mb-4" />
            <p className="text-[var(--color-text-primary)] font-medium">Processing your signature...</p>
          </div>
        </div>
      )}
    </div>
